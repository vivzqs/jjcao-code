...
#include <DGAL/Polyhedron_decorator.h>
#include <DGAL/Polyhedron_subdivision_rules.h>
#include <DGAL/Polyhedron_memory_builder.h>
...
/** Subdivide the input polyhedron p with Loop subdivision 
      rules.note: the polyhedron should be parameterized firstly.
  */
  static void Loop_Param_subdivision(Polyhedron& p, int step = 1) {
    tri_quadralize_polyhedron_with_param(p, Loop_Param_rule<Polyhedron>() , step);
  }

 /** Subdivide the input polyhedron p with Modify_Butterfly subdivision 
      rules.
  */
  static void Modify_Butterfly_subdivision(Polyhedron& p, int step = 1) {
    tri_quadralize_polyhedron(p, Modify_Butterfly_rule<Polyhedron>() , step);
  }

  /** Subdivide the input polyhedron p with Modify_Butterfly subdivision 
      rules.note: the polyhedron should be parameterized firstly.
  */
  static void Modify_Butterfly_Param_subdivision(Polyhedron& p, int step = 1) {
    tri_quadralize_polyhedron_with_param(p, Modify_Butterfly_Param_rule<Polyhedron>() , step);
  }
  ...
  static void tri_quadralize_polyhedron(Polyhedron& p, RULE<Polyhedron> rule, int step = 1) {
   ...
   ///
  template <template <typename> class RULE>
  static void tri_quadralize_polyhedron_with_param(Polyhedron& p, RULE<Polyhedron> rule, int step = 1) {
    for (int i = 0; i < step; i++) tri_quadralize_with_param_1step(p, rule);
  }
...
//@}
    template <template <typename> class RULE>
  static void tri_quadralize_with_param_1step(Polyhedron& p, RULE<Polyhedron> rule);
...
// ======================================================================
///subdivision with parameter together
// ======================================================================
///
template <class _P> template <template <typename> class RULE>
void Polyhedron_subdivision<_P>::tri_quadralize_with_param_1step(_P& p, RULE<_P> rule) {
  p.normalize_border();
  ...
  //u, v
  Point* vertex_uv_buffer = new Point[num_vertex + num_edge];
  Point* edge_uv_buffer = vertex_uv_buffer + num_vertex;
  ...
  {	  
	  ...
	  rule.edge_point_rule(eitr, edge_point_buffer[i], edge_uv_buffer[i]);
  }
  ...
  for (int i = num_edge-sb; i < num_edge; i++, ++eitr) 
  {
	  ...
	  v_onborder[v] = true;
	  ///?
	  rule.border_point_rule(eitr, edge_point_buffer[i], vertex_point_buffer[v],vertex_uv_buffer[v],edge_uv_buffer[i]);
  }
  ...
  for (int i = 0; i < num_vertex; i++, ++vitr)
	  ...
	  //?
	  if (!v_onborder[i]) rule.vertex_point_rule(vitr, vertex_point_buffer[i],vertex_uv_buffer[i]);
  ...
  for (int i = 0; i < num_edge; i++, ++eitr) {
	  Vertex_handle vh = insert_vertex(p, eitr);
	  vh->point() = edge_point_buffer[i];
	  vh->uv(edge_uv_buffer[i][0],edge_uv_buffer[i][1]);
	//  vh->point() = edge_uv_buffer[i];
  }
  Facet_iterator fitr = p.facets_begin();
  for (int i = 0; i < num_facet; i++, ++fitr) {
	  // Step 2.
  ...
  for (int i = 0; i < num_vertex; i++, ++vitr)
  {
	  vitr->point() = vertex_point_buffer[i];
	  //  vitr->setuv()
	  vitr->uv(vertex_uv_buffer[i][0], vertex_uv_buffer[i][1]);

  }


  delete []vertex_uv_buffer;
  delete []vertex_point_buffer;
}
